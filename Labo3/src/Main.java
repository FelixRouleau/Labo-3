...
public class Main {
    ...
    public static void listerTout() {
        ...
        Set<Entry<Integer, Vol>> mappings = listHashMapTrie.entrySet(); // Returns the ordered TreeMap too a Set (So no
                                                                        // duplicate) in ascending order
        nbVols = 0;
        afficherEntete();
    ...
    public static void ajouterVol() {
        ...
        numVol = Integer.parseInt(JOptionPane.showInputDialog(null, "Entrez le numéro de vol à ajouter",
            ...
            switch (typeVol) {
                ...
                case 2:
                    option = JOptionPane.showConfirmDialog(null, message, "AJOUT D'UN VOL",
                            JOptionPane.OK_CANCEL_OPTION);
                    if (option == JOptionPane.OK_OPTION) {
                        // Random un type d'avion régulier pour le vol régulier. Soit l'index 1 ou 4 du
                        // ArrayList flotteCompagnie
                        Random r = new Random();
                        ...
                        listeMapVols.put(numVol,
                                new VolRegulier(numVol, destination.getText(), dateDepart,
                                        Integer.parseInt(nbRes.getText()),
                                        flotteCompagnie.get(randomRegularFlight[indexAvion])));
                    }
                    break;
                case 3:
                    option = JOptionPane.showConfirmDialog(null, message, "AJOUT D'UN VOL",
    ...
    public static void modifierDate() {
        ...
        numVol = Integer
                .parseInt(JOptionPane.showInputDialog(null, "Entrez le numéro de vol où vous désirez changez la date.",
                        "AJOUT D'UN VOL", JOptionPane.PLAIN_MESSAGE));
        if (rechercherVol(numVol)) {
    ...
    public static void reserverVol() {
        ...
        numVol = Integer.parseInt(JOptionPane.showInputDialog(null, "Entrez le numéro de vol où vous désirez réserver.",
                "RÉSERVATION", JOptionPane.PLAIN_MESSAGE));
        if (rechercherVol(numVol)) {
            msg += listeMapVols.get(numVol).getNumVol();
            ...
            if (option == JOptionPane.OK_OPTION) {
                nbReserve = Integer.parseInt(JOptionPane.showInputDialog(null, "Combien de siège voulez vous réservé?",
                        "RÉSERVATION", JOptionPane.PLAIN_MESSAGE));
